...
export const login_patch_validate = async (
  ...
  if (account){
    ...
    if (!check_account) data_error.message.push("Tài khoản hoặc mật khẩu không hợp lệ!");
    else{
      const check_password = await bcrypt_compare(
        password,
        check_account.password
      );
      if(check_password == false) data_error.message.push("Tài khoản hoặc mật khẩu không hợp lệ!");
    }
  }
